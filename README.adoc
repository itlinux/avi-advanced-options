...
** *ssh controller*
** shell into controller *(shell --u admin)*
...
** *mkdir /tmp/demo* (create a folder to host the output of the convert script)
** cd to _/opt/avi/python/lib/avi/migrationtools/ansible_
** then execute the convert script from json into ansible
** *python3 avi_config_to_ansible.py -c /tmp/demo.json  -o /tmp/demo*
** *execute and see what needs to be address, you probably have some options that are not needed*
